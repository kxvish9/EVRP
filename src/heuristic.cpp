...
static bool repair_tour(int *tour, int &size)
{
    ...
    for (int i = 0; i < size - 2; i++)
    {
        ...
        if (current_energy + get_energy_consumption(from, to) > BATTERY_CAPACITY)
        {
            ...
            {
                if (get_energy_consumption(from, cs_id) <= current_energy)
                { // Can we reach the station?
...
void run_heuristic()
{
    ...
    {
        if (current_tour_size <= 3)
        {
            continue; // Not enough nodes to perform a valid 2-Opt swap
        }
